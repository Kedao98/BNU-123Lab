...
def draw_bboxes(image, bboxes, line_thickness):
    ...
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        ...
        t_size = cv2.getTextSize(f"{cls_id} ID-{pos_id}", 0, fontScale=line_thickness / 3, thickness=font_thickness)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(image, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(image, f"{cls_id} ID-{pos_id}", (c1[0], c1[1] - 2), 0, line_thickness / 3,
...
def update(bboxes, image):
    bbox_xywh, confs, bboxes2draw = [[] for _ in range(3)]

    for x1, y1, x2, y2, lbl, conf in bboxes:
        obj = [int((x1 + x2) * 0.5), int((y1 + y2) * 0.5), x2 - x1, y2 - y1]
        bbox_xywh.append(obj)
        confs.append(conf)

    xywhs, confss = torch.Tensor(bbox_xywh), torch.Tensor(confs)
